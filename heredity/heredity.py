...
def num_gene(person, one_gene, two_genes):
...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    probability = 1
    for person in people:
        gene_num = num_gene(person, one_gene, two_genes)
        trait_val = person in have_trait
        trait_prob = PROBS['trait'][gene_num][trait_val]
        ...
        if not mother or not father:
            ...
            if gene_num == 2:
                ...
                probability *= ((1 - mother_prob) * father_prob) +\
...
def normalize(probabilities):
    ...
    """
    for person in probabilities:
        for event in probabilities[person]:
            total = sum(probabilities[person][event].values())
            for value in probabilities[person][event]:
                val = probabilities[person][event][value]
                normalized_value = val/total
                probabilities[person][event][value] = normalized_value
