...
class CrosswordCreator():
        ...
        for domain in assignment:
            ...
            for neighbor in self.crossword.neighbors(domain):
                if neighbor in assignment:
                    overlap = self.crossword.overlaps[domain, neighbor]
    ...
    def select_unassigned_variable(self, assignment):
        ...
        return values.
        """
        unassigned_var = set(self.domains.keys()) - set(assignment.keys())
        min_domain = float('inf')
        max_degree = 0
        best_variable = None
        for variable in unassigned_var:
            variable_domain = len(self.domains[variable])
            if self.crossword.neighbors(variable) is not None:
                variable_degree = len(self.crossword.neighbors(variable))
            else:
                variable_degree = 0
            if min_domain > variable_domain:
                min_domain = variable_domain
                best_variable = variable
                max_degree = variable_degree
            if min_domain == variable_domain:
                if max_degree < variable_degree:
                    max_degree = variable_degree
                    best_variable = variable
